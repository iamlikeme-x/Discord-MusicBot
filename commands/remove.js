...
const { TrackUtils } = require("erela.js");
   ...
   */
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      let remove = Number(args[0].value);
      if (!player.queue || !player.queue.length || player.queue.length === 0)
      return client.sendTime("❌ | **Nothing is playing right now...**");
      let rm = new MessageEmbed()
        .setDescription(`✅ | **Removed track** \`${remove}\` from the queue!`)
        .setColor("GREEN")
      if (isNaN(remove)) rm.setDescription(`**Usage:** \`${GuildDB.prefix}remove [track]\``);
      if (remove > player.queue.length)
        rm.setDescription(`The queue has only ${player.queue.length} songs!`);
      await interaction.send(rm);
      player.queue.remove(remove - 1);
    },
  }
};
